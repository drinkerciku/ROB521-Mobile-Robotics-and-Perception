...
for i=2:numodom
    ...
    % obtain control inputs and dt between time stamps
    currU = [v_odom(i - 1); omega_odom(i - 1)];
    dt = t_true(i) - t_i;

    % generate matric G(q)
    G_q = [cos(q_i(3)) 0; sin(q_i(3)) 0; 0 1];
    q_i = q_i + dt*G_q*currU;

    % make sure that theta remains in the range [-pi, pi]
    if (q_i(3) > 0) && (q_i(3) > pi)

        q_i(3) = q_i(3) - 2*pi;

    elseif (q_i(3) < 0) && (q_i(3) < -pi)
...
for n=1:2
    ...
    for i=1:size(t_laser,1)
        ...
        if abs(omega_interp(i)) < 0.1
            % extract data from the i-th reading
            Rlaser_i = y_laser(i,:);
            % transform in x,y coordinates in robots local frame
            QLaser_i = [Rlaser_i.*cos_angles; Rlaser_i.*sin_angles];
    
            % generate rotation matrix from local to global frame
            R_i = [cos(-theta_interp(i)), sin(-theta_interp(i));
                   -sin(-theta_interp(i)), cos(-theta_interp(i))];
            
            % map to the gloabl reference frame
            QGlobal = R_i*(QLaser_i - [0.1; 0]) + [x_interp(i); y_interp(i)];
            plot(QGlobal(1,:), QGlobal(2,:), colour)
        end
