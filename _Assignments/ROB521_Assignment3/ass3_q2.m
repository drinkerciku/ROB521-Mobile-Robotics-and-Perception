...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % compute bearing angle in robot's frame of reference
            phi_j_particle = angles(j);

            % initialize varibles to 0
            xGrid = 0;
            yGrid = 0;
            R = 0;

            % iterate through viable ranges towards the bearing angle
            % direction
            for r = r_min_laser : ogres : r_max_laser

                % pose transformation to global coordinates and grid (x,y)
                rParticle = r*[cos(phi_j_particle); sin(phi_j_particle)] - [0.1; 0];
                rGlobal = T*[rParticle; 1];

                xGrid = round((rGlobal(1) - ogxmin)/ogres);
                yGrid = round((rGlobal(2) - ogymin)/ogres);
                ...
                % check the value of the occupancy map
                if ogp(yGrid, xGrid) >= 0.5
                    R = r;
                    break
                end
            ...
            % compute gain 
            w_particle(n) = w_gain*w_particle(n)*normpdf(y_laser(i,j),...
                    R, sqrt(laser_var));
