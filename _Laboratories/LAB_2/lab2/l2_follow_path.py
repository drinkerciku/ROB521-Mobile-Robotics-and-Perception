...
class PathFollower():
    ...
    def points_to_robot_circle(self, points):
        ...
        radRes = int(COLLISION_RADIUS/self.map_resolution) # TODO: check if self map resolution is legit
        ...
        for i in range(0, cellPos.shape[1]):
            ...
            if keyLoc not in pts2Rob:
                ...
                tempDiff[0, tempDiff[0] >= self.map_np.shape[0]] = self.map_np.shape[0] - 1
                tempDiff[0, tempDiff[1] >= self.map_np.shape[1]] = self.map_np.shape[1] - 1
    ...
    def checkCollision(self, points):
        ...
        pts2RobPoses = points_to_robot_circle(points) # dictionary with occupancy regions
        ...
        for key in pts2RobPoses:
            ...
            mapOcc = pts2RobPoses[key] # coordiantes based on set passed in by "points"
            ...
            crash = np.any(self.map_np[mapOcc[1, :], self.map_shape[0] - mapOcc[0, :]] == 0) # TODO: ensure this is how map_np works
            if crash:
                return True
    ...
    def calculate_cost(self, cand_opt, end_pt):
        ...
        loc_cost = trans_factor*pose_diff[0:2]
        rot_cost = rot_factor*pose_diff[3]
    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            print("DONE: Propogate the trajectory forward, storing the resulting points in local_paths!")
            # propogate trajectory forward, assuming perfect control of velocity and no dynamic effects
            for i, opts in enumerate(self.all_opts_scaled):
                curr_pred_loc = local_paths[t-1,i]
                trans_vel = opts[0]
                rot_vel = opts[1]
                next_pred_loc = trajectory_rollout(trans_vel, rot_vel, curr_pred_loc)
                local_paths[t,i] = next_pred_loc
            ...
            print("DONE: Check the points in local_path_pixels for collisions")
            for opt in range(local_paths_pixels.shape[1]):
                # for timestep in range(local_paths_pixels.shape[0]):
                rob_center_pixels = local_path_pixels(opt)
                print ("trajetory center pixels")
                print (rob_center_pixels)
                if checkCollision(rob_center_pixels): # if there is a collision anywhere along timstep
            ...
            print("DONE: Remove trajectories with collisions!")

            # calculate final cost and choose best option
            print("DONE: Calculate the final cost and choose the best control option!")
            # final_cost = np.zeros(self.num_opts)
            final_cost = np.zeros(len(valid_opts))
            for i in valid_opts:
                cur_opt = self.all_opts_scaled[i]
                final_cost[i] = calculate_cost(cur_opt)
