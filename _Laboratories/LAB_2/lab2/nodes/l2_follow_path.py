...
ROT_GOAL_TOL = .6 #.6  # rad, tolerance to consider a goal complete
...
class PathFollower():
    def __init__(self):
        ...
        self.map_nonzero_idxes = np.argwhere(self.map_np) #prob use this for colision detection
        # print(map.info)
        # print(np.shape(self.map_np))

        # for debug
        self.map_xy = np.nonzero(self.map_np)
        # print(map_xy)
        ...
        self.path_tuples = np.load(os.path.join(cur_dir, 'RRT_star_willow_seed_7.npy')).T
