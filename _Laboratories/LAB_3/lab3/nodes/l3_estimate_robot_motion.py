...
PHI_COL = lambda leftE, rightE : np.array([[rightE], [leftE]])
G_Q_MATRIX = np.array([[WHEEL_RADIUS/2 , WHEEL_RADIUS/2],
                       [WHEEL_RADIUS/(2*BASELINE), -WHEEL_RADIUS/(2*BASELINE)]])
B_Q = lambda robHead : np.array([[np.cos(robHead), 0],
                                 [np.sin(robHead), 0],
                                 [0, 1]])
...
class WheelOdom:
    ...
    def sensor_state_cb(self, sensor_state_msg):
        ...
        if self.last_enc_l is None:
            ...
            # difference of ticks and time step
            diffL = self.safeDelPhi(self.last_enc_l, le)
            diffR = self.safeDelPhi(self.last_enc_r, re)

            print(le, re)

            dt = (sensor_state_msg.header.stamp- self.last_time).to_sec()

            # arc angle from the encoder
            dotPhi_L = 2*np.pi*diffL/ENC_TICKS
            dotPhi_R = 2*np.pi*diffR/ENC_TICKS

            # compute velocities
            currQuart = self.pose.orientation
            # get currHead
            _, _, currHead = euler_from_ros_quat(currQuart)
            print(currHead)

            thetaB = B_Q(currHead)
            enc = PHI_COL(dotPhi_R, dotPhi_L)
            dotQ = np.dot(thetaB, np.dot(G_Q_MATRIX, enc))

            # save new pose
            self.pose.position.x += dotQ[0]
            self.pose.position.y += dotQ[1]
            # deal with quartenion
            currAngle = currHead - dotQ[2]
            newQ = ros_quat_from_euler([0, 0, currAngle])
            self.pose.orientation = newQ

            self.twist.linear.x = dotQ[0]/dt
            self.twist.linear.y = dotQ[1]/dt
            self.twist.angular.z = dotQ[2]/dt
            ...
            # save the current reading to use for next callback
            self.last_enc_l = sensor_state_msg.left_encoder
            self.last_enc_r = sensor_state_msg.right_encoder
            self.last_time = sensor_state_msg.header.stamp
            ...
            print("Wheel Odom: x: %2.3f, y: %2.3f, theta: %2.3f, dt: %2.3f" % (
                self.pose.position.x, self.pose.position.y, euler_from_ros_quat(self.pose.orientation)[2], dt
            ))
